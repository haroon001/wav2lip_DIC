...
from data_dic import util_dataset
...
parser.add_argument(
        '--sr_opt', type=str, required=True, help='Path to options JSON file.')

args = parser.parse_args()
args.img_size = 96

sr_opt = option.parse(parser.parse_args().sr_opt)
sr_opt = option.dict_to_nonedict(sr_opt)
...
def main():
  ...
  for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, total=int(np.ceil(float(len(mel_chunks))/batch_size)))):
    if i == 0:
      ...
      print ("Model loaded")
      solver = SRLandmarkGANSolver(sr_opt)

      norm = transforms.Normalize((0.509 * sr_opt['rgb_range'], 
                                          0.424 * sr_opt['rgb_range'], 
                                          0.378 * sr_opt['rgb_range']), 
                                         (1.0, 1.0, 1.0))
    ...
    for p, f, c in zip(pred, frames, coords):
      ...
      if not args.no_sr:
        lr_tensor = util_dataset.np2Tensor([p], sr_opt['rgb_range'])[0]
        lr_tensor = norm(lr_tensor)
        
        p_tensor = torch.unsqueeze(lr_tensor, dim = 0).cuda()
        solver.LR.resize_(p_tensor.size()).copy_(p_tensor)
        solver.test()
